 ...
 * It implements the AIHeader interface, in order to follow the structure of an AI class.
 */
public class Random_AI extends AbstractGamePlayer implements AIHeader{
    ...
    public Move request() {
        ...
        if (gameBoard.getDepositedCards().isEmpty() || Math.random() < 0.5) {   //if the deposit area is empty or 50% probability -> take from drawpile_stack
            ...
            to_take = cardFromDeposit();            //else we know, that the deposited area is not empty --> take from there
        }

        //----------------------------------
        if (Math.random() < 1.0/ (configuration.getNumCardsPerPlayerHand()+1)) {
            to_deposit = to_take;                   //by the probability of 1/owncards+1 -> deposited card is the taken card
        }
        
        else {
            to_deposit = removeFromOwn(to_take);       //else a card from own
        }
